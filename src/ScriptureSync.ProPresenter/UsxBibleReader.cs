...
{
    public static int GetBookIndex(string book) => Array.IndexOf(Names, book);
    public static string GetBookCode(string book)
    {
        var index = GetBookIndex(book);
        return index >= 0 ? Codes[index] : throw new InvalidDataException($"Unsupported canonical book: {book}");
    }
